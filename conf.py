...
# Bootstrap Icons SVGs (https://icons.getbootstrap.com/) for the footer.
# Furo's `footer_icons` slot replaces Pydata's `icon_links` and requires
# inline SVG markup rather than icon-font class names.
...
#
# Replaces the previous _templates/layout.html approach, which doesn't
# work under Furo. The html-page-context hook appends to the `metatags`
# context variable, which both Furo and most other Sphinx themes render
# inside <head>.
